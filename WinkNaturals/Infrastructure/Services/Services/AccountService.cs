    ...
    public class AccountService : IAccountService
    {
        ...
        public async Task<bool> MakeCreditCardAsPrimary(int customerID, CreditCard card, CreditCardType type)
        {
            try
            {
                ...
                //delete cards 
                await _customerService.DeleteCustomerCreditCard(customerID, CreditCardType.Primary.ToString());
                await _customerService.DeleteCustomerCreditCard(customerID, CreditCardType.Secondary.ToString());
               
                // Validate that we have a token
                var token = card.Token;
                if (string.IsNullOrEmpty(token)) return false;

                //Save primary credit card
                var savePrimaryCard = new SetAccountCreditCardTokenRequest
                {
                    CustomerID = customerID,
                ...
                await _exigoApiContext.GetContext(false).SetAccountCreditCardTokenAsync(savePrimaryCard);

                //Save secondary credit card
                var secondaryCardDetail = (CreditCard)existingPrimaryCard; 
                var saveSecondaryCard = new SetAccountCreditCardTokenRequest
                {
                    CustomerID = customerID,
                    CreditCardAccountType = AccountCreditCardType.Secondary,
                    CreditCardToken = secondaryCardDetail.Token,
                    ExpirationMonth = secondaryCardDetail.ExpirationMonth,
                    ExpirationYear = secondaryCardDetail.ExpirationYear, 
                    BillingName = secondaryCardDetail.NameOnCard,
                    BillingAddress = secondaryCardDetail.BillingAddress.AddressDisplay,
                    BillingCity = secondaryCardDetail.BillingAddress.City,
                    BillingState = secondaryCardDetail.BillingAddress.State,
                    BillingZip = secondaryCardDetail.BillingAddress.Zip,
                    BillingCountry = secondaryCardDetail.BillingAddress.Country
                };
                await _exigoApiContext.GetContext(false).SetAccountCreditCardTokenAsync(saveSecondaryCard);
                return true;
            }
            catch (Exception ex)
            {
